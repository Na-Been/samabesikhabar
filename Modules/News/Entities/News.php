...
//    }
    ...
    public function getFullFeatureImageAttribute()
    {
        if ($this->feature_image != null) {
            return url('/') . Storage::url($this->feature_image);
        } else {
            return asset('frontend/no-image.png');
        }
    }

    public function getFullAuthorImageAttribute()
    {
        if ($this->author_image != null) {
            return url('/') . Storage::url($this->author_image);
        } else {
            return asset('frontend/samabesi_logo1.png');
        }
    }
