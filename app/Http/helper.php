...
function storeNewImageFile($request)
{
    $fileName = explode('storage/', $request->image);
    return $fileName[1];
}

function updateImageFile($request)
{
    $fileName = explode('storage/', $request->image);
    if (count($fileName) == 1) {
        return $fileName[0];
...
{
    return NewsCategory::where('category_id', $category_id)->with(['news', 'category'])->take(10)->get()->sortByDesc('news.created_at');
}
...
function getSixteenCategory(): Collection
{
    $ninth = Category::where('order', 16)->first();
    return $ninth ? getNewsByCategory($ninth->id) : collect();
}
...
function getLatestSixNews(): Collection
{
    return News::latest()->take(18)->get() ?? collect();

}

function getTrendingNews(): Collection
{
    return News::orderBy('share', 'DESC')->take(6)->get();
}
...
function getAdsForHomePage()
{
    return Advertisement::where(['page' => 1, 'status' => 1])->get();
}
